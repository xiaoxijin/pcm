...
class RemoteShell
{
    ...
    static function listen($serv, $host = "127.0.0.1", $port = 9599)
    {
        ...
        $port->on("Connect", '\Server\RemoteShell::onConnect');
        $port->on("Close", '\Server\RemoteShell::onClose');
        $port->on("Receive", '\Server\RemoteShell::onReceive');
        $serv->on("PipeMessage", '\Server\RemoteShell::onPipeMessage');
        self::$serv = $serv;
    }
