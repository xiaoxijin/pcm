...
class Parser
{
    ...
    function parseHeader($data)
    {
        ...
        unset($headerLines[0]);
        //解析Header
        $header = array_merge($header, $this->parseHeaderLine($headerLines));
        return $header;
    }
    ...
    {
        $request->cookie = $this->parseParams($request->header['Cookie']);
    }
    ...
    function parseFormData($request, $cd)
    {
        ...
        {
            if ($f === '') continue;
            $parts = explode(self::HTTP_EOF, trim($f));
            $header = $this->parseHeaderLine($parts[0]);
            if (!isset($header['Content-Disposition'])) continue;
            $meta = $this->parseParams($header['Content-Disposition']);
            //filename字段表示它是一个文件
    ...
    function parse($data)
    {
        ...
        $header = $this->parseHeader($_header);
        if ($header === false)
        {
            $this->isError = true;
        }
