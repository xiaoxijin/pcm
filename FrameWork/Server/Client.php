...
class Client
{
    ...
    public function getStat($ip = "", $port = "")
    {
        ...
        $sendData = \Xphp\Pack\DoraPacket::packEncode($packet);
        $result = $this->doRequest($sendData, DoraConst::SW_MODE_WAITRESULT_SINGLE);

        if ($this->guid != $result["guid"]) {
            return \Xphp\Pack\DoraPacket::packFormat("guid wront please retry..", 100100, $result["data"]);
        }
    ...
    private function doRequest($sendData, $type)
    {
        ...
        try {
            ...
            $data = \Xphp\Pack\DoraPacket::packFormat($e->getMessage(), $e->getCode());
            $data["guid"] = $this->guid;
            return $data;
        }
        ...
        if (!$ret) {
            ...
            if ($errorcode == 0) {
                $msg = "connect fail.check host dns.";
                $errorcode = -1;
                $packet = \Xphp\Pack\DoraPacket::packFormat($msg, $errorcode);
            } else {
                $msg = socket_strerror($errorcode);
                $packet = \Xphp\Pack\DoraPacket::packFormat($msg, $errorcode);
            }
    ...
    private function waitResult($client)
    {
        while (1) {
            ...
            if ($result !== false && $result != "") {
                $data = \Xphp\Pack\DoraPacket::packDecode($result);
                ...
                //time out
                $packet = \Xphp\Pack\DoraPacket::packFormat("the recive wrong or timeout", 100009);
    ...
    public function getAsyncData()
    {
            ...
            if (count(self::$asynclist) > 0) {
                foreach (self::$asynclist as $k => $client) {
                    if ($client->isConnected()) {
                        $data = $client->recv();
                        if ($data !== false && $data != "") {
                            $data = \Xphp\Pack\DoraPacket::packDecode($data);
                            ...
                            self::$asynresult[$k] = \Xphp\Pack\DoraPacket::packFormat("the recive wrong or timeout", 100009);
                            continue;
                        }
                    } else {
                        //remove the result
                        unset(self::$asynclist[$k]);
                        self::$asynresult[$k] = \Xphp\Pack\DoraPacket::packFormat("Get Async Result Fail: Client Closed.", 100012);
                        continue;
                    }
                } // foreach the list
            } else {
        ...
        return \Xphp\Pack\DoraPacket::packFormat("OK", 0, $result);
    }
