...
class RemoteShell
{
    ...
    static function listen($serv, $host = "127.0.0.1", $port = 9599)
    {
        ...
        $port->on("Connect", '\Xphp\Server\RemoteShell::onConnect');
        $port->on("Close", '\Xphp\Server\RemoteShell::onClose');
        $port->on("Receive", '\Xphp\Server\RemoteShell::onReceive');
        $serv->on("PipeMessage", '\Xphp\Server\RemoteShell::onPipeMessage');
        self::$serv = $serv;
    }
