...
class Tool
{
    ...
    static function getNextEditStatus($updateTime,$cycle = 30*24*3600,& $next_edit_date){
        ...
        {
            $updateTime_vals = array_values($updateTime);
            sort($updateTime_vals);
            $updateTime = $updateTime_vals[0];
        }

        if($updateTime+ $cycle<__TIME){
            return true;//可修改
        }else{
            $next_edit_date= date('Y/m/d', $updateTime+ $cycle);
            return false;//不可修改
        }
    ...
    static function httpExpire($lastModifyTime, $expire = 1800)
    {
        ...
        {
            \Xphp::$xphp->http->header($key, $value);
        }
        return $result;
    }
    /**
    ...
    static public function listFiles($path,& $files=array()){
        $dir = @ dir($path);
...

        {
            if(is_dir($path."/".$file) AND ($file!=".") AND ($file!="..")) {
                ...
                self::listSubDir($path."/".$file,$files);
            } else {
                $files[]=$path."/".$file;
            }
        }
    ...
    static public function listSubDir($path,& $files=array()) {
