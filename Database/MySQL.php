 ...
 *
 */
class MySQL implements \IFace\Database
{
    public $debug = false;
    public $conn = null;
    public $config;
    ...
    function connect()
    {
        ...
        if (!$this->conn)
        {
            \Exception\Error::info(__CLASS__." SQL Error", mysql_error($this->conn));
            return false;
        }
        mysql_select_db($db_config['name'], $this->conn) or \Exception\Error::info("SQL Error", mysql_error($this->conn));
        if ($db_config['setname'])
        {
            mysql_query('set names ' . $db_config['charset'], $this->conn) or \Exception\Error::info("SQL Error",
...
class MySQLRecord implements \IFace\DbRecord
{
    public $result;
